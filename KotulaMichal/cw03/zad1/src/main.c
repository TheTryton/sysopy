...
void fork_print_dir_info(const char* dir)
{
    pid_t pid = fork();
    if(pid == 0)
    {
        printf("relative_path=%s pid=%i ", dir, getpid());
        fflush(stdout);
        system("ls -l");
        exit(0);
    } else{
...
void traverse_a_impl(const char* dir_path, int current_depth, input_arguments args)
{
    ...
    while((entry = readdir(dir)) != NULL)
    {
        ...
        // if file is a directory, traverse recursively and run a subprocess

        if(entry->d_type == DT_DIR)
        {
            fork_print_dir_info(path);
...
void traverse_a(input_arguments args)
{
    ...
    fork_print_dir_info(args.dir_path);
